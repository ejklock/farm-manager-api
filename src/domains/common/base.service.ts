...
export default class BaseService<T extends BaseEntity> {
  ...
  public async findOneOrFail(id: number): Promise<T | null> {
    this.logger.log(`Find one ${id}`);
    const options: FindOneOptions<T> = {
      where: { id } as unknown as FindOneOptions<T>['where'],
    };
    ...
    if (!item) {
      const nameOfEntity = this.repository.metadata.targetName;
      throw new NotFoundException(`${nameOfEntity} not found with id ${id}`);
    }
    return item;
  }
  ...
  public async update(id: number, item: Partial<T>): Promise<T> {
    ...
    const itemToUpdate = await this.findOneOrFail(id);
    if (!itemToUpdate) {
